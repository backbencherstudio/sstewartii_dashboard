# Sidebar Generator (current dashboard pattern)
# Usage:
#   .\scripts\createSidebar.ps1
#   .\scripts\createSidebar.ps1 -TargetPath "components/layout/Sidebar"
#   .\scripts\createSidebar.ps1 -Force

param(
    [string]$TargetPath = "components/layout/Sidebar",
    [switch]$Force
)

$projectRoot = Split-Path $PSScriptRoot -Parent
$sidebarPath = Join-Path $projectRoot $TargetPath

if (-not (Test-Path $sidebarPath)) {
    New-Item -ItemType Directory -Path $sidebarPath -Force | Out-Null
    Write-Host "Created folder: $TargetPath"
}
else {
    Write-Host "Folder exists: $TargetPath"
}
...
foreach ($fileName in $files.Keys) {
    $path = Join-Path $sidebarPath $fileName

    if ((Test-Path $path) -and (-not $Force)) {
        Write-Host "Skipped existing file: $TargetPath/$fileName (use -Force to overwrite)"
        continue
    }

    $files[$fileName] | Set-Content -Path $path -Force
    Write-Host "Written: $TargetPath/$fileName"
}

Write-Host "`nDone. Sidebar scaffold is ready."